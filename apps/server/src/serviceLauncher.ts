...
// Mirrors pinnedRuntimePaths: a runtime is an npm-installed kca-code version
// whose bin.mjs runs on the machine's Node. Kept inline so this file stays on
// Node built-ins only.
...
export class Launcher {
  ...
  async #startChild(version: string, role: ChildRole, update?: ServiceUpdateRecord): Promise<void> {
    ...
    if (!(await runtimeExists(this.#baseDir, version))) {
      throw new Error(`Selected kca-code@${version} runtime is missing or incomplete.`);
    }
    if (this.#stopping) return;
