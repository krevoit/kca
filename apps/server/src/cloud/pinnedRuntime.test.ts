  ...
  ProcessRunner.ProcessRunner.of({
    run: (input) =>
      Effect.gen(function* () {
        ...
        const entry = path.join(stagingDir, "node_modules", "kca-code", "dist", "bin.mjs");
...
it.layer(NodeServices.layer)("ensurePinnedRuntimeInstalled", (it) => {
  it.effect("installs kca-code from npm as the runtime executable", () =>
    Effect.gen(function* () {
      ...
      assert.equal(
        paths.entryPath,
        path.join(paths.versionDir, "node_modules", "kca-code", "dist", "bin.mjs"),
      );
  ...
  it.effect("refuses a failed npm install without publishing a runtime", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const baseDir = yield* fs.makeTempDirectoryScoped({ prefix: "t3-pinned-npm-bad-" });
      const error = yield* ensurePinnedRuntimeInstalled({
        ...
        runner: ProcessRunner.ProcessRunner.of({
          run: () =>
            Effect.succeed({
              stdout: "npm ERR! registry unreachable",
              stderr: "",
