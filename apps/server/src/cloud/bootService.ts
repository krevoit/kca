...
export function formatBootServiceProblem(problem: BootServiceProblem): string {
  switch (problem) {
    ...
    case "restart-pending":
      return "A newer version is installed but the service is still running the previous one. Run `kca service restart` to switch.";
  }
}
...
>()("kca-code/cloud/bootService") {}

export interface BootServiceHost {
  readonly execPath: string;
  /** Overrides where the stable launcher copy is read from (tests). */
...
export const make = Effect.fn("cloud.boot_service.make")(function* (input: {
  ...
  // KCA fork: the unit runs a stable launcher copy under node, not the
  // versioned install (self-update replaces version dirs under it).
  ...
  // The unit runs the stable launcher copy under the installing Node, which
  // supervises the pinned kca-code runtime and survives version switches.
  ...
  const install = Effect.fn("cloud.boot_service.install")(function* (options?: {
    ...
    const installed = yield* fs
      .exists(unitPath)
      .pipe(Effect.mapError((cause) => new BootServiceInstallError({ cause })));
    // Read after the runtime install: the default launcher source lives
    // inside the versioned runtime tree.
