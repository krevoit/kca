...
const linuxLauncher = "/home/theo/.t3/runtime/service-launcher.mjs";
const linuxPlan = {
  program: ["/usr/bin/node", linuxLauncher],
  baseDir: "/home/theo/.t3",
  logPath: "/home/theo/.t3/userdata/logs/boot-service.log",
  unitPath: "/home/theo/.config/systemd/user/kca.service",
};
...
it("keeps systemd pinned to the stable launcher rather than a versioned server", () => {
  ...
  expect(unit).toContain(`ExecStart=/usr/bin/node ${linuxLauncher}`);
  expect(unit).toContain("KillMode=mixed");
});

it("reads the served KCA home back out of a rendered unit or plist", () => {
...
const macPlan = {
  ...
  unitPath: "/Users/theo/Library/LaunchAgents/com.t3tools.kca.service.plist",
};
...
const makeHarness = Effect.fn("test.make_boot_service_harness")(function* (
  ...
  // The install copies the stable launcher out of the runtime tree.
  yield* fs.writeFileString(
    path.join(path.dirname(runtime.entryPath), SERVICE_LAUNCHER_FILE),
    "export {};\n",
  );
  ...
  const runner = ProcessRunner.ProcessRunner.of({
    run: Effect.fn("test.run_boot_service_command")(function* (
      ...
      if (!failed && command === "loginctl enable-linger --no-ask-password 501")
        control.linger = "yes";
      if (!failed && command === "systemctl --user enable kca.service") control.enabled = true;
      if (!failed && command === "systemctl --user restart kca.service") control.active = true;
      if (
        control.stateAfterStop !== undefined &&
        (command === "systemctl --user stop kca.service" ||
        ...
        stdout:
          input.args[0] === "--version"
            ? // The runtime under test reports the version of the directory it
              // was launched from, like the real CLI.
              `kca v${/versions\/([^/]+)\//.exec(input.command)?.[1] ?? "1.2.3"}\n`
  ...
  const makeService = (
    ...
    Effect.gen(function* () {
      ...
      yield* fs.writeFileString(
        path.join(path.dirname(paths.entryPath), SERVICE_LAUNCHER_FILE),
        "export {};\n",
      );
...
it.layer(NodeServices.layer)("boot service install", (it) => {
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        expect(commands).not.toContain("systemctl --user stop kca.service");
      }),
  );
  ...
  it.effect("installs, reports current state, and uninstalls", () =>
    Effect.gen(function* () {
      const { service, fs, statePath, timeouts } = yield* makeHarness();
      const path = yield* Path.Path;
      const plan = yield* service.install();
      ...
      expect(plan.program).toEqual([
        "/usr/bin/node",
        path.join(plan.baseDir, "runtime", SERVICE_LAUNCHER_FILE),
      ]);
      expect(yield* fs.readFileString(plan.unitPath)).toContain(
        `ExecStart=/usr/bin/node ${path.join(plan.baseDir, "runtime", SERVICE_LAUNCHER_FILE)}`,
      );
      expect(yield* service.status).toMatchObject({
        current: true,
      ...
      expect(timeouts.get("systemctl --user disable --now kca.service")).toEqual(
    ...
    (platform) =>
      Effect.gen(function* () {
        ...
        expect(
          ...
          platform === "linux"
            ? ["systemctl --user stop kca.service", "systemctl --user restart kca.service"]
            : [
                "launchctl bootout --wait gui/501/com.t3tools.kca.service",
  ...
  it.effect("restart stops and starts an installed service, and is a no-op otherwise", () =>
    Effect.gen(function* () {
      ...
      expect(
        ...
        "systemctl --user stop kca.service",
        "systemctl --user daemon-reload",
        "systemctl --user enable kca.service",
        "systemctl --user restart kca.service",
      ]);
    }),
  );
  ...
  it.effect("restarts without overwriting a pending remote update", () =>
    Effect.gen(function* () {
      ...
      for (const allowDowngrade of [false, true]) {
        ...
        ).toEqual(["systemctl --user stop kca.service", "systemctl --user restart kca.service"]);
      }
    }),
  );
    ...
    Effect.gen(function* () {
      ...
      expect(
        plan.unitPath.endsWith(
          path.join("Library", "LaunchAgents", "com.t3tools.kca.service.plist"),
        ),
      ).toBe(true);
      expect(yield* fs.readFileString(plan.unitPath)).toContain(
      ...
      expect(yield* fs.readFileString(plan.unitPath)).toContain(
        `    <string>/usr/bin/node</string>\n    <string>${path.join(plan.baseDir, "runtime", SERVICE_LAUNCHER_FILE)}</string>`,
      );
      expect(yield* service.status).toMatchObject({
        current: true,
        installedVersion: "1.2.3",
      ...
      expect(timeouts.get("launchctl bootout --wait gui/501/com.t3tools.kca.service")).toEqual(
        Duration.seconds(120),
      );
    }),
  );
  ...
  it.effect("restarts the launch agent when repair fails", () =>
    Effect.gen(function* () {
      ...
      expect(commands.filter((command) => command.startsWith("launchctl "))).toEqual([
        "launchctl bootout --wait gui/501/com.t3tools.kca.service",
        "launchctl enable gui/501/com.t3tools.kca.service",
  ...
  it.effect("ignores a bootout for an agent that is not loaded", () =>
    Effect.gen(function* () {
      ...
      yield* service.install();
      control.failCommand = "launchctl bootout --wait gui/501/com.t3tools.kca.service";
