...
it.layer(NodeServices.layer)("bin cli parsing", (it) => {
  ...
  it.effect("rejects connect commands when public configuration is missing", () =>
    Effect.gen(function* () {
      ...
      assert.deepEqual(error.commandPath, ["kca", "connect"]);
  ...
  it.effect("exposes service lifecycle commands without T3 Connect configuration", () =>
    Effect.gen(function* () {
      ...
      assert.include(output, "Manage the KCA background service.");
  ...
  it.effect("reports actionable human-readable headless connect state", () =>
    Effect.gen(function* () {
      ...
      assert.include(output, "Next: Run `kca connect link` to authorize and enable T3 Connect.");
    }),
  );
  ...
  it.effect("rejects invalid ttl values before running auth commands", () =>
    Effect.gen(function* () {
      ...
      assert.deepEqual(error.commandPath, ["kca", "auth", "pairing", "create"]);
  ...
  it.effect("rejects dev-url on project commands", () =>
    Effect.gen(function* () {
      ...
      assert.deepEqual(error.commandPath, ["kca", "project", "add"]);
