...
const serviceInstallCommand = Command.make("install", serviceReconcileFlags).pipe(
  Command.withDescription("Install KCA as a background service for this user."),
...
const serviceUninstallCommand = Command.make("uninstall", projectLocationFlags).pipe(
  Command.withDescription("Stop and remove the KCA background service."),
  Command.withHandler((flags) =>
    runServiceCommand(
      flags,
      Effect.gen(function* () {
        ...
        yield* Console.log(removed ? "Removed the KCA service." : "KCA service is not installed.");
      }),
    ),
  ),
);
...
const serviceStatusCommand = Command.make("status", projectLocationFlags).pipe(
  Command.withDescription("Show whether the KCA background service is installed."),
  Command.withHandler((flags) =>
    runServiceCommand(
      flags,
      Effect.gen(function* () {
...
export const offerServiceDuringOnboarding = Effect.gen(function* () {
  ...
  if (installed && current) {
    yield* Console.log("KCA is already set up to run in the background on this machine.");
    return true;
  }
  ...
  if (
    ...
    yield* Console.log(
      `A newer kca@${status.installedVersion} background service is installed. Leaving it unchanged.`,
    );
  ...
  const wanted = yield* Prompt.run(
    Prompt.confirm({
      message: installed
        ? "The installed KCA service needs an update or repair. Update it now?"
        : platform === "darwin"
          ? "Run KCA in the background whenever you log in to this Mac? " +
            "It stays reachable through T3 Connect while you are logged in."
          : "Run KCA in the background whenever this machine boots? " +
