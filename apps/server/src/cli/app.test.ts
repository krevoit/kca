    ...
    withTempDirectory("t3-app-ssh-test-", (root) =>
      Effect.gen(function* () {
        ...
        expect(error).toMatchObject({
          ...
          message:
            "`kca app` only controls a desktop app on the same machine. It cannot run over SSH.",
        });
    ...
    withTempDirectory("t3-app-platform-test-", (root) =>
      Effect.gen(function* () {
        ...
        expect(error).toMatchObject({
          ...
          platform: "freebsd",
          message: "`kca app` is not supported on freebsd.",
        });
        expect(yield* pathExists(baseDir)).toBe(false);
      }),
    ),
    ...
    withTempDirectory("t3-app-missing-test-", (root) =>
      Effect.gen(function* () {
        ...
        expect(error).toMatchObject({
          ...
          message: expect.stringContaining("Could not reach the KCA desktop app."),
          cause: { code: "ENOENT" },
        });
        expect(yield* pathExists(baseDir)).toBe(false);
      }),
