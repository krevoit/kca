/**
 * All text `kca triage` hands to the coding agent. Kept as bare template strings
...
export const TRIAGE_PLAYBOOK = `# KCA triage playbook

You are a support engineer for KCA (https://github.com/krevoit/kca), working
...
Fetch https://raw.githubusercontent.com/krevoit/kca/main/.github/triage/PLAYBOOK.md.
...
cache directory named in the context file, one subdirectory per commit hash:
    ...
    git clone --depth 1 --filter=blob:none --branch <release-tag> \\
      https://github.com/krevoit/kca <source-cache-dir>/<hash>
...
- How is KCA running on this machine: \`npx kca-code serve\` in a terminal, the
...
Search existing issues in krevoit/kca (use \`gh\`, or the public GitHub search
...
Do not patch the KCA source as a fix. A good issue with strong repro steps
...
- If \`gh\` is not authenticated, offer \`gh auth login\`, or build a prefilled
  https://github.com/krevoit/kca/issues/new URL with title and body query
...
export const buildTriageSeedPrompt = (contextFilePath: string) => `A KCA user is \
having a problem with their install and started this session with \`kca triage\`.
...
export const buildTriageContext = (input: TriageContextInput) => `# KCA triage context

Generated by \`kca triage\` at ${input.generatedAt}.
