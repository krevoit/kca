...
/**
 * `kca theme` - inspect and set the environment's theme. Connected web and
 ...
 * Settings afterwards sticks until the next `kca theme set`.
 *
...
export class ThemeTargetMissingError extends Schema.TaggedErrorClass<ThemeTargetMissingError>()(
  ...
  override get message(): string {
    return "Provide a theme id or file, or run `kca theme clear` to remove the theme.";
  }
}
...
const themeSetCommand = Command.make("set", {
  ...
  Command.withHandler((flags) =>
    Effect.gen(function* () {
      ...
      // make `kca theme set ocean` publish ./ocean whenever the cwd happens to
