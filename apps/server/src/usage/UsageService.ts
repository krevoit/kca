...
export const make = Effect.gen(function* () {
  ...
  const resolveTranscriptDirs = Effect.fn("UsageService.resolveTranscriptDirs")(function* (
        ...
        if (driver === "codex") {
          ...
          if (!seen.has(archiveKey)) {
            ...
            dirs.push({
              provider,
              dir: archiveDir,
              volumeId: yield* Effect.promise(() => readDirectoryVolumeId(archiveDir)),
            });
          }
        }
      }
    ...
    dirs.push({
      provider: "opencode",
      dir: opencodeDataDir,
      volumeId: yield* Effect.promise(() => readDirectoryVolumeId(opencodeDataDir)),
  ...
  const collectDirs = Effect.fn("UsageService.collectDirs")(function* (
    ...
    for (const { provider, dir, volumeId, fileName, dbFileName } of dirs) {
  ...
  const scanSummary = Effect.fn("UsageService.scanSummary")(function* (
    ...
    for (const { provider, dir, volumeId, files, failure } of scannedDirs) {
      ...
      sources.push({
        ...
        // KCA fork: a read failure (e.g. an unreadable OpenCode database)
        // still surfaces as failed rather than missing.
        status:
          failure !== null ? "failed" : files === null && scannedFiles === 0 ? "missing" : "ok",
        scannedFiles,
        skippedFiles,
        malformedRecords: 0,
