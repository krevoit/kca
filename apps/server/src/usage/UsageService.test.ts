  ...
  ServerConfig.layerTest(process.cwd(), { prefix: input.prefix }).pipe(
    ...
    Layer.provideMerge(
      Layer.succeed(HostProcessEnvironment, {
        ...
        // Isolate OpenCode discovery from the developer's real database:
        // without an override it would scan ~/.local/share/opencode.
        XDG_DATA_HOME: input.opencodeDataDir ?? NodePath.join(input.home, "xdg-data-missing"),
        ...input.environment,
      }),
    ),
  );
      ...
      Effect.gen(function* () {
        ...
        yield* Effect.gen(function* () {
          ...
          assert.include(
            ...
            NodePath.join(NodeFS.realpathSync(configured), "projects"),
          );
          ...
          assert.include(
            ...
            NodePath.join(NodeFS.realpathSync(environmentHome), "projects"),
          );
        }).pipe(
          Effect.provide(
            serviceLayers({
  ...
  it.live("scans every configured Codex home plus archived sessions", () =>
    Effect.gen(function* () {
      ...
      assert.deepStrictEqual(codexDirs, [
        NodePath.join(NodeFS.realpathSync(personalHome), "archived_sessions"),
        NodePath.join(NodeFS.realpathSync(personalHome), "sessions"),
        NodePath.join(NodeFS.realpathSync(workHome), "sessions"),
      ]);
    }).pipe(Effect.scoped),
  );
  ...
  it.live("does not share an in-flight scan after custom prices change", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.gen(function* () {
        ...
        let homeProbes = 0;
        const expectedClaudeProjects = NodeFS.realpathSync(
          ...
          Effect.provideService(FileSystem.FileSystem, {
            ...
            exists: (path) =>
              fileSystem.exists(path).pipe(
                Effect.tap(() => {
                  // realPath-normalized: the service resolves symlinked
                  // parents (macOS /var -> /private/var) before probing.
                  let resolved: string | null = null;
                  try {
                    resolved = NodeFS.realpathSync(path);
                  } catch {
                    return Effect.void;
                  }
                  if (resolved !== expectedClaudeProjects) return Effect.void;
                  homeProbes += 1;
