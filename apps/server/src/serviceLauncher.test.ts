...
// A pinned runtime is an npm-installed kca-code tree: the launcher spawns
// <versionDir>/node_modules/kca-code/dist/bin.mjs beside the install-complete
// sentinel, so the tests stand one up as a Node shebang script with the
// executable bit set, the way npm installs the real bin.
const writeFakeRuntime = (
  ...
  Effect.gen(function* () {
    const entryPath = path.join(versionDir, "node_modules", "kca-code", "dist", "bin.mjs");
...
it.layer(NodeServices.layer)("service state persistence", (it) => {
  ...
  it.effect("a fresh launcher clears a restart deferred by service update", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
