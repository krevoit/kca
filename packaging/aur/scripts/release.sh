...
version="${tag#v}"
pkgver="${version//-/_}"
asset_name="KCA-${version}-x86_64.AppImage"
...
cd "$aur_dir"
git rm --ignore-unmatch LICENSE .upstream-commit t3code-icon.png kca-icon.png
git config user.name 'kca-ci'
git config user.email 'kca-ci@users.noreply.github.com'
git add -A
