...
export interface RemoteT3RunnerOptions {
  /**
   * KCA fork: npm package spec (e.g. `kca-code@nightly`) the remote installs
   * and runs. The fork's remote path; needs Node on the remote. Takes effect
   * only when no archive version is set.
   */
...
function isArchiveRunner(runner: RemoteT3RunnerOptions | undefined): boolean {
  // Archive installs bring their own runtime (no Node); npm-spec and
  // node-script runners use the node-based helpers instead.
  if (runner?.nodeScriptPath?.trim()) return false;
  return Boolean(runner?.archiveVersion?.trim());
}
...
function sshRunnerLogFields(runner: RemoteT3RunnerOptions | undefined) {
  ...
  if (runner?.packageSpec?.trim()) {
    return { runner: "npm", packageSpec: runner.packageSpec.trim() };
  }
...
// KCA fork: npm-registry install branch for the remote runner. Rendered only
// when the caller passes a package spec and no archive version (see
// buildRemoteT3RunnerScript). Unlike the archive branch below it needs Node
// on the remote, matching the fork's documented SSH requirements.
const REMOTE_NPM_RUNNER_BRANCH = `T3_PACKAGE_SPEC=@@T3_PACKAGE_SPEC@@
if command -v kca >/dev/null 2>&1; then
  exec kca "$@"
fi
# npm extracts a package before it runs the native builds of its dependencies,
# so a failed build leaves the npx cache without a kca executable. \`npx --yes\`
# then exits 0 without running anything at all, which the caller only ever sees
# as a server that never becomes ready. Resolve the CLI once up front so that
# install failure is reported here, with npm's output on stderr.
require_installed_kca_cli() {
  if ! KCA_CLI_PATH="$("$@" -- sh -c 'command -v kca')"; then
    printf 'Remote host could not install %s. See npm output above for the cause.\\n' @@T3_PACKAGE_SPEC@@ >&2
    return 1
  fi
  if [ -n "$KCA_CLI_PATH" ]; then
    return 0
  fi
  printf 'Remote host installed %s but npm produced no kca executable, which usually means a native dependency failed to build. Install a C toolchain on the remote host (Debian/Ubuntu: build-essential, Fedora/RHEL: gcc-c++ make, macOS: xcode-select --install) and try again.\\n' @@T3_PACKAGE_SPEC@@ >&2
  return 1
}
# The launcher records this PID, so exec the CLI without an npm wrapper process.
if command -v npx >/dev/null 2>&1; then
  require_installed_kca_cli npx --yes --package @@T3_PACKAGE_SPEC@@ || exit 1
  exec "$KCA_CLI_PATH" "$@"
fi
if command -v npm >/dev/null 2>&1; then
  require_installed_kca_cli npm exec --yes --package @@T3_PACKAGE_SPEC@@ || exit 1
  exec "$KCA_CLI_PATH" "$@"
fi
printf 'Remote host is missing the kca CLI and could not install @@T3_PACKAGE_SPEC@@ because node/npm/npx are unavailable on PATH. Install Node or configure a supported version manager for non-interactive shells.\\n' >&2
exit 1
`;
...
@@T3_NPM_BRANCH@@T3_ARCHIVE_VERSION=@@T3_ARCHIVE_VERSION@@
...
export class SshMissingRunnerError extends Schema.TaggedError<SshMissingRunnerError>()(
  ...
  override get message(): string {
    return "A remote runner needs a package spec, an archive version, or a node script path.";
  }
}

export function buildRemoteT3RunnerScript(input?: RemoteT3RunnerOptions): string {
  // KCA fork: the npm-spec install is the default remote path (the fork ships
  // kca-code on npm, not self-contained archives). An explicit archive version
  // still selects upstream's archive install instead.
  ...
  return stripTrailingNewlines(
    applyScriptPlaceholders(REMOTE_RUNNER_SCRIPT, {
      ...
      T3_NPM_BRANCH:
        packageSpec === "" || archiveVersion !== ""
          ? ""
          : applyScriptPlaceholders(REMOTE_NPM_RUNNER_BRANCH, {
...
export function buildRemoteLaunchScript(input?: RemoteT3RunnerOptions): string {
  return applyScriptPlaceholders(REMOTE_LAUNCH_SCRIPT, {
    T3_ARCHIVE_MODE: isArchiveRunner(input) ? "1" : "0",
    T3_NODE_ENV_SCRIPT: buildRemoteNodeEnvScript(input),
