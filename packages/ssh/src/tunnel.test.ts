...
describe("ssh tunnel scripts", () => {
  ...
  it("installs and runs a package spec with npx and npm fallbacks (KCA)", () => {
    const script = buildRemoteT3RunnerScript({ packageSpec: "kca-code@nightly" });

    assert.include(script, "T3_PACKAGE_SPEC='kca-code@nightly'");
    assert.include(script, "T3_NODE_SCRIPT_PATH=''");
    assert.include(script, "T3_ARCHIVE_VERSION=''");
    assert.include(script, 'exec kca "$@"');
    assert.include(script, 'exec "$KCA_CLI_PATH" "$@"');
    assert.include(script, "could not install 'kca-code@nightly'");
    assert.include(script, "require_installed_kca_cli npx --yes --package 'kca-code@nightly'");
    assert.include(script, "require_installed_kca_cli npm exec --yes --package 'kca-code@nightly'");
    assert.include(script, "npm produced no kca executable");
    assert.notInclude(script, "t3@nightly");
    assert.notInclude(script, 'exec t3 "$@"');

    const launch = buildRemoteLaunchScript({ packageSpec: "kca-code@nightly" });
    assert.include(launch, "T3_ARCHIVE_MODE=0");
  });
